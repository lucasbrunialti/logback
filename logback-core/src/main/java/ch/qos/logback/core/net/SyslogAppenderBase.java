...
public abstract class SyslogAppenderBase<E> extends AppenderBase<E> {
	...
	final static String SYSLOG_LAYOUT_URL = CoreConstants.CODES_URL
			+ "#syslog_layout";
	final static int MAX_MESSAGE_SIZE_LIMIT = 256 * 1024;
	final static int DEFAULT_RECONNECTION_DELAY = 30000;
	final static int MAX_BUFFER_MSG_SIZE = 1000;
	...
	CircularFifoQueue<E> eventQueue;
	...
	public void start() {
		...
		try {
			...
			addWarn("Failed to connect socket. Will try to reconnect later.", e);
			fireConnector();
		}
	...
	protected void append(E eventObject) {
		...
		if (eventQueue == null) {
			eventQueue = new CircularFifoQueue<E>(MAX_BUFFER_MSG_SIZE);
		}
		...
		if (connector == null) {
			while (eventQueue.isEmpty() == false) {
				try {
					...
					if (msg.length() > MAX_MESSAGE_SIZE_LIMIT) {
						msg = msg.substring(0, MAX_MESSAGE_SIZE_LIMIT);
					}
	...
	/**
	 * The port number on the syslog server to connect to. Normally, you would
	...
	public void stop() {
		try {
			...
			addWarn("Could not close socket connection.", ioe);
		}
		super.stop();
	}
	...
	class Connector extends Thread {
		...
		public void run() {
			while (!interrupted) {
				...
				// if we get here, we failed to connect. we'll give up on the
				// queued events
				eventQueue = null;
			}
		}
